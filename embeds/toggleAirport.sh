...
eth_names=$(/usr/sbin/networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: (.*Ethernet|USB 10.*|AX88179A.*|Thunderbolt Ethernet .*), Device: (en[0-9]+)\)$/\2/p')
air_name=$(/usr/sbin/networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: (Wi-Fi|AirPort), Device: (en.)\)$/\2/p')
